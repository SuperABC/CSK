...
科研：回合开始时，你可以获得一名距离1以内角色的全部手牌，若本回合你对其造成了伤害则回合结束时你将全部手牌交给该角色。

名目：锁定技，你无法成为手牌数大于你当前绩点的角色“别学了”和“决斗”的目标。

##### 3 蔡展璋 4 ok (群体伤害，弃置他人牌)
...
##### 5 朱瑞昇 4 ok (防御判定，减少受伤)

神速：当你的绩点下降时，你可以弃置你判定区内一张牌。
...
天秀：始终视为有对象，当其他角色脱单后，你可以提升一个绩点或者摸一张牌。

##### 7 王宜霖 3 ok (使用他人手牌，限制距离)

绝世：每回合限一次，你可以声明一种牌，然后选择一名角色的手牌，若其为所声明的牌，则视为使用此牌，否则此牌置入弃牌堆。
...
外交：出牌阶段限一次，你可以摸一张牌，然后弃置X张牌并交换另外两名角色的手牌（X为另外两名角色手牌数之差）。

联衡：锁定技，当你的手牌数小于你的绩点时，你摸牌至使手牌数与绩点相等。
...
支书：你的别学了均视为党性压制，其他角色对你的党性压制均视为别学了。
...
膜蛤：出牌阶段限一次，你可以失去一个绩点并摸一张牌，然后使另一名角色提升一个绩点。

##### 19 宋鼎 3 ok (额外摸牌，转移伤害)

博学：摸牌阶段开始时，你可以放弃摸牌，改为展示牌堆顶5张牌，之后获得其中每种花色的牌各一张，其余的置入弃牌堆

耽美：当被一名男性角色指定为别学了的目标时，你可以弃置一张牌，然后将此别学了转移给另一名其攻击范围内的其他男性角色。

##### 20 叶靖琛 4 ok (额外摸牌，装备转移)

叶落：当你受到伤害时，可以选择摸一张牌，若你有装备，可以同时弃置一张装备牌并摸一张牌。
...
好学：锁定技，不可被指定为Steam好友。

演说：你可以将两张相同颜色的牌当做没学使用。
...
##### 25 张程易 4 ok (得判定牌，防御技巧牌)

闻名：当其他角色的延时技巧牌因判定失效时，你可以获得该技巧牌。

大爷：你可以将一张技巧牌当做滚使用。

##### 26 朱淞伯 3 ok (高防御，装备转化)
...
##### 29 王逸旦 3 ok (弃他人装备，提升绩点)

褪裙：当你对一名男性角色造成伤害时，可以弃置一张手牌，然后弃置该男性角色装备区内一张牌。

给力：出牌阶段，你可以弃置两张手牌，然后指定一名男性角色，然后你与该男性角色都提升一点绩点。

##### 30 吴朝晖 4 ok (群体伤害，置换手牌)
...
习蛤：其他角色对你造成伤害的时候，你可以弃置全部手牌，然后防止此伤害。

心望：当你受到伤害时，你可以摸牌至手牌数与全场手牌最多的人相同。

##### 36 郑虎 7 ok (提升绩点上限)

群主：每回合限一次，当被黑色牌指定为对象之一时，可以损失一个绩点，取消该牌的作用并将该牌置于人物牌上，记为群费。
